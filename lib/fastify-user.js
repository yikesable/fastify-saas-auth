...
// TODO: Verify that fastifySecureSession has been registered Eg:
...
//     .register(fp(addHooks))
// }
// TODO: Add auth handler at `onRequest` or `preParsing` https://github.com/fastify/fastify-secure-session/blob/fdc0242c14797fbc47bfa2d6dc003c4f5c43acfb/index.js#L231
...
const fastifyUserPluginFactory = async (fastify, options) => {
  ...
  fastify.decorate('requireAuthenticated', function (condition) {
    this.addHook('preValidation', async (request, reply) => {
      if (request.user && (!condition || await condition(request))) {
        // Its all okay!
        return;
      }
