...
const SESSION_KEY_USER_ID = 'fastify:user:user-id';

/** @typedef {import('./advanced-types.d.ts').FastifyUserObject} FastifyUserObject */
/** @typedef {(userId: string) => Promise<Partial<FastifyUserObject|undefined>>} LoadUser */

/**
 * @typedef FastifyUserOptions
 * @property {{ set: (key: any, value: any) => void, delete: (key: any) => void }} [fastifyContext]
 * @property {boolean} [getUserIdFromHeader]
 * @property {LoadUser} [loadUser]
 */
...
const fastifyUserPluginFactory = async (fastify, options) => {
  ...
  /** @type {WeakMap<import('fastify').FastifyRequest, FastifyUserObject|undefined>} */
  ...
  const loadLoggedInUser = async function (request, userId, { skipLoading } = {}) {
    /** @type {FastifyUserObject|undefined} */
    let userDefinition;

    if (userId) {
      const loadedUser = loadUser && !skipLoading ? await loadUser(userId) : { skippedLoading: !!skipLoading };
    ...
    if (fastifyContext) fastifyContext.set('user', userDefinition);
  };
  ...
  const removeLoggedInUser = function () {
    ...
    if (fastifyContext) fastifyContext.delete('user');
  };

  fastify.addHook('onRequest', async (request) => {
    const userId = getUserIdFromHeader
      ? request.headers['x-test-user-id'] // Usable in tests
  ...
  fastify.decorateRequest('user', {
    /** @type {(this: import('fastify').FastifyRequest) => FastifyUserObject | undefined} */
    getter () {
...
export const fastifyUserPlugin = fp(fastifyUserPluginFactory, { fastify: '>=3.x' });
