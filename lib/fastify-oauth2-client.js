...
/** @type {import('fastify').FastifyPluginAsync<import('./advanced-types.js').FastifyOAuth2ClientOptions>} */
const fastifyOAuth2ClientPluginFactory = async (fastify, options) => {
  ...
  fastify.get('/' + pathModule.relative(baseUrl.pathname, redirectUri.pathname), async function (request, reply) {
    // TODO [@fastify/oauth2@>7.3.0]: Use the new prefixed values: https://github.com/fastify/fastify-oauth2/pull/224
    /** @type {import('@fastify/oauth2').OAuth2Namespace|undefined} */
    // @ts-expect-error
    const oauth2 = this[name];
    ...
    /** @type {import('./advanced-types.js').SaasAuthIssuerUserInfo|undefined} */
    let userinfo;
