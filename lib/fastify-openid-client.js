...
/** @import { BaseParams, SessionValue, FastifyOpenIdClientOptions } from './fastify-openid-client-types.js' */

const { OPError, RPError } = openIdClientErrors;
const { codeChallenge, random } = openIdClientGenerators;

export const SESSION_KEY_PREFIX_OPENID = 'fastify:oidc:';

/** @type {FastifyPluginAsync<FastifyOpenIdClientOptions>} */
const fastifyOpenIdClientPluginFactory = async (fastify, options) => {
  ...
  const sessionKey = rawSessionKey || `${SESSION_KEY_PREFIX_OPENID}${name}`;
  ...
  fastify.get('/' + pathModule.relative(baseUrl.pathname, redirectUri.pathname), async (request, response) => {
    ...
    try {
      ...
      success = await callback(userinfo, { name, request });
    } catch (err) {
